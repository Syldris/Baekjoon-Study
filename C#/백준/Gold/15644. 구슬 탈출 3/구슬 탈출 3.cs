...
public class Program
{
    static void Main()
    {
        ...
        Queue<(int rx, int ry, int bx, int by, int count, List<int> list)> queue = new Queue<(int, int, int, int, int, List<int> list)>();
        queue.Enqueue((redPos.x, redPos.y, bluePos.x, bluePos.y, 1, new List<int>()));

        while (queue.Count > 0)
        {
            (int rx, int ry, int bx, int by, int count, List<int> list) = queue.Dequeue();
            ...
            for (int dir = 0; dir < 4; dir++)
            {
                ...
                if (!visited[curRedx,curRedy,curBluex,curBluey])
                {
                    ...
                    newlist.Add(dir);
                    queue.Enqueue((curRedx, curRedy, curBluex, curBluey, count + 1, newlist));
                }
            }
        }
