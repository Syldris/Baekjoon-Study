...
class Program
{
    static void Main()
    {
        ...
        while (true)
        {
            ...
            int range = m * 2; // 음수 1~m 양수 m+1~2m 각각 관리해야해서 1~2n까지 필요
            ...
            for (int i = 1; i <= m; i++)
            {
                ...
                {
                    sw.WriteLine(0); // 증상이 참(x)이여도 죽고 없어도(-x) 죽는다. 
                    die = true;
                    break;
                }
            }
            ...
            void DFS(int node)
            {
                ...
                // DFS상에서 a => b 식으로 탐색하니 단방향은 완료.
                // 만약 b에서 a를 가는 우회로를 발견하면 a <=> b 양방향 싸이클로 묶을수있는것.
            ...
            int Setting(int x)
            {
                if (x < 0) // 시작이 음수면 1~m으로 설정.
                    return -x;
                else       // 시작이 양수면 m+1~2m으로 설정.
                    return x + m;
            }

            int Convert(int x) // 음수 양수 전환
            {
                if (x <= m) // 1~m 이면 음수이니 +m으로 양수로 만들어줌.
                    return x + m;
                else       // m+1~2m이면 양수이니 -m으로 음수로 만들어줌.
                    return x - m;
            }
        }
    }
