...
## 🔍 트리에서의 다이나믹 프로그래밍 관련문제
> 트리 구조에서 다이나믹 프로그래밍(DP)을 적용하는 대표 유형을 정리함

### 🧩 가중치 있는 독립 집합 (Weighted Independent Set)

트리에서 **인접한 정점을 동시에 선택할 수 없는** 문제 유형
...
**DP 상태 정의**
  - `dp[node][0]` → 현재 정점을 선택하지 않은 경우
  - `dp[node][1]` → 현재 정점을 선택한 경우
...
✅ 이 유형은 **역추적을 통해 실제 선택된 정점 목록 복원이 가능**(2213번)

---

### 🌳 서브트리 정보 누적 유형

트리에서 **서브트리 단위 정보를 누적 계산**하는 패턴
