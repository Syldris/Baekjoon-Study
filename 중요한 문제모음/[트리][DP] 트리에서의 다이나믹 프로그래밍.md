...
* **공간 복잡도**: `O(N)`
  ...
  * 인접 리스트, DP 테이블, 재귀 스택(또는 명시적 스택) 등 사용.
...
3. 각 노드에서 자식들의 DP값을 모아 자신의 DP 상태를 계산한다.
4. 루트의 DP 값으로 전체 최적해를 판단하거나, 루트 기준으로 도출한 값을 사용해 답을 얻는다.
...
문제 유형에 따라 상태 정의는 달라지지만, 자주 쓰이는 전형적인 패턴은 다음과 같다.
...
* `dp[node][1]` = node의 값 + (모든 자식에 대해 자식이 선택되지 않은 경우의 값 합)
* `dp[node][0]` = (모든 자식에 대해 자식 선택/비선택 중 최대값 합)

일반식(문장으로 표현):

> 부모를 선택하면 자식은 무조건 비선택(=dp[child][0]만 가능).
> 부모를 비선택하면 자식은 선택하거나 비선택할 수 있으므로 각 자식마다 `max(dp[child][0], dp[child][1])`를 더한다.

---

## ✅ 자주 쓰이는 상태/전이의 변형 예시(문제 유형별 포인트)
...
* **서브트리의 정점 갯수**
  ...
  * 상태: 서브트리에 포함된 정점 개수
  * 전이: 부모는 자식 서브트리 정점을 합산

> 핵심은 “서브트리에서 무엇을 의미 있는 상태로 저장할지”가 핵심이다.
...
2. 각 상태에 대한 **전이 규칙(문장)**을 만든다.

   * 예: "`dp[u][1]`를 선택한다면 u의 모든 자식 v는 `dp[v][0]`만 가능"

3. 트리를 루트로 고정하고 **후위 DFS** 흐름을 그린다.
...
* 상태 정의가 문제 해결의 핵심이며, 전이식은 상태의 의미에서 자연스럽게 나온다.
* 구현 주의사항(부모 체크, 자료형, 재귀 깊이 등)을 미리 점검하면 디버그 시간을 많이 절약할 수 있다.
