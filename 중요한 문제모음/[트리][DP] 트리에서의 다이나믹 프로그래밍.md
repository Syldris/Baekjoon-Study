...
* `dp[node][1]` → 현재 정점을 선택했으므로 **자식은 전부 비선택 상태만 가능**
* `dp[node][0]` → 현재 정점을 선택하지 않았으므로  각 자식은 **선택 / 비선택 중 최대값을 자유롭게 선택 가능**
...
* 상태 정의가 문제 해결의 핵심이며, 전이식은 상태의 의미에서 알아낼 수 있다.
